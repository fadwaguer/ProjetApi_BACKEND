...
const getAllComponents = async (req, res) => {
  try {
    ...
    const formatted = components.map((component) => ({
      _id: component._id,
      name: component.name,
      category: component.category?.name || component.category,
      brand: component.brand,
      specs: component.specs,
      image: component.image?.data
        ? `data:${
            component.image.contentType
          };base64,${component.image.data.toString("base64")}`
        : null,
    }));
...
const getComponentsByCategory = async (req, res) => {
  try {
    ...
    if (components.length === 0) {
      return res
        .status(404)
        .json({ message: "Aucun composant trouvé dans cette catégorie." });
    }
    ...
    }).populate("partner", "name image");

    // Associer les prix aux composants
    const formattedComponents = components.map((component) => {
      const componentPrices = prices
        .filter(
          (price) => price.component.toString() === component._id.toString()
        )
        .map((price) => ({
      ...
      const image =
        component.image && component.image.data
          ? `data:${
              component.image.contentType
            };base64,${component.image.data.toString("base64")}`
          : null;
...
const getComponentDetails = async (req, res) => {
  try {
    ...
    const prices = await Price.find({ component: id }).populate(
      "partner",
      "name image"
    );
    ...
    res.status(500).json({
      message:
        "Une erreur est survenue lors de la récupération des détails du composant.",
      error: error.message,
    });
  }
};
...
const addComponent = async (req, res) => {
  try {
    ...
    if (!name || !category) {
      return res
        .status(400)
        .json({ message: "Le nom et la catégorie du composant sont requis." });
    }
    ...
    if (existingComponent) {
      return res
        .status(400)
        .json({ message: "Le composant existe déjà dans cette catégorie." });
    }
    ...
    if (typeof specs === "string") {
      try {
        ...
        return res.status(400).json({
          message: "Le format des spécifications est invalide.",
          error: error.message,
        });
      }
    }
...
const updateComponent = async (req, res) => {
  try {
    ...
    if (name || categoryExists) {
      ...
      if (existingComponent) {
        return res.status(400).json({
          message: "Un composant avec ce nom existe déjà dans cette catégorie.",
        });
      }
    }
